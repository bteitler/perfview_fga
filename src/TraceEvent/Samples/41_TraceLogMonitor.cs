...
namespace TraceEventSamples
{
    ...
    internal class TraceLogMonitor
    {
        ...
        public static void Run()
        {
            ...
            // Repeat the test a bunch of times to stress the robustness of the
            // teardown logic, since the native pointer management can be fragile.
            const int TEST_REPEAT_COUNT = 200;
            for (int i = 0; i < TEST_REPEAT_COUNT; i++)
            {
                ...
                using (session = new TraceEventSession("TraceLogSession"))
                {
                    ...
                    {
                        // benteitler: Turn on experimental optimizations that improve performance but
                        // there may be stability issues in some scenarios.
                        traceLogSource.TraceLog.ExperimentalPerfOptimizations = true;
